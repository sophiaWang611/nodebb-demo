...
var userDigest = require('../service/user/actions/digest');
var userEmail = require('../service/user/actions/email');
...
var getAdminSearchDict = require('../service/admin/search').getDictionary;
...
SocketAdmin.restart = function (socket, data, callback) {
	async.waterfall([
		function (next) {
			require('../service/meta/actions/build').buildAll(next);
		},
		function (next) {
			events.log({
				type: 'build',
...
SocketAdmin.plugins.toggleActive = function (socket, plugin_id, callback) {
	require('../service/posts/actions/cache').reset();
...
SocketAdmin.settings.clearSitemapCache = function (socket, data, callback) {
	require('../common/sitemap').clearCache();
	callback();
};
