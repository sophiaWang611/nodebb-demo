...
module.exports = function (db, module) {
	var helpers = module.helpers.mysql;
	...
	pubsub.on('mysql:hash:cache:del', function (key) {
		cache.del(key);
	});

	pubsub.on('mysql:hash:cache:reset', function () {
		cache.reset();
	});

	module.delObjectCache = function (key) {
		pubsub.publish('mysql:hash:cache:del', key);
		cache.del(key);
	};

	module.resetObjectCache = function () {
		pubsub.publish('mysql:hash:cache:reset');
		cache.reset();
	};
