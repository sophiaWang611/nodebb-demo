...
function completeConfigSetup(config, next) {
	...
	async.waterfall([
		...
		function (next) {
			require('../common/database').init(next);
		},
		function (next) {
			require('../common/database').createIndices(next);
		},
	], next);
}
...
function createAdministrator(next) {
	var Groups = require('../service/groups/index');
...
function createMenuItems(next) {
	...
	db.exists('navigation:enabled', function (err, exists) {
		...
		var navigation = require('../service/navigation/admin');
