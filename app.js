...
var prestart = require('./src/cli-source/prestart');
...
if (nconf.get('setup') || nconf.get('install')) {
	...
	async.series([
		...
		require('./src/service/meta/actions/build').buildAll,
	], function (err) {
		if (err) {
			throw err;
		}
	...
	require('./src/service/meta/actions/build').build(nconf.get('build'));
} else if (nconf.get('events')) {
	require('./src/cli/manage').listEvents();
} else {
	require('./src/cli-source/start').start();
}
